import { clear, getAll, getKeys, getValue, removeValue, setValue } from './inbox-storage';
...
export const addMessage = async () => {
  ...
  if (messages.length < messagePool.length) {
    const dt = Date.now();
    await setValue(dt, { ...messagePool[messages.length], date: format(dt, 'MMM dd, yyyy h:mmaa') });
  }
};

export const getMessages = async (): Promise<EmailMessage[]> => [...(await getAll())].reverse().map((x) => x.value);

export const getMessage = async (idx: number): Promise<EmailMessage | null> => {
  const keys = [...(await getKeys())].reverse();
  return idx < keys.length ? getValue(keys[idx]) : null;
};

export const markRead = async (idx: number): Promise<void> => {
  const keys = [...(await getKeys())].reverse();
  if (idx < keys.length) {
...
export const removeAllMessages = async (): Promise<void> => {
  await clear();
};

export const removeMessage = async (): Promise<void> => {
  const keys = await getKeys();
  if (keys.length) {
    await removeValue(keys[keys.length - 1]);
  }
};
